...
function* fetchRecipes() {
  let recipes = yield fetch(BaseUrl)
  ...
  else yield put({ type: actions.FETCH_RECIPES_FAILED, payload: [] });
}
...
function* updateRecipe(action) {
  ...
  let update = yield fetch(BaseUrl, options)
    .then(response => response.json())
    .catch(reason => undefined);

  if (update)
    yield put({ type: actions.UPDATE_RECIPE_SUCCEED, payload: update });
  else yield put({ type: actions.UPDATE_RECIPE_FAILED, payload: null });
}

function* deleteRecipe(action) {
  let options = { method: "DELETE" };
  let succeed = yield fetch(`${BaseUrl}/${action.payload}`, options)
    .then(response => true)
    .catch(reason => false);
