...
const recipesReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case actions.SELECT_RECIPE:
            ...
            return {
                ...state,
                selectedRecipe: action.payload !== '' ? state.items.filter((x) => x.id === action.payload)[0] : null,
                loading: false,
            };
        ...
        case actions.DELETE_RECIPE_SUCCEED:
            return { ...state, loading: false, items: state.items.filter(x => x.id !== action.payload) };
