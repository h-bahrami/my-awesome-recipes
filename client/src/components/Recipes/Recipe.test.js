...
describe('Testing Recipes Component', () => {
    ...
    it('test click event (using render from enzyme)', async () => {
        ...
        const wrapper = mount((<Recipe recipe={recipe} onChange={mockUpdateFunc} onCancel={mockCancelFunc} />));

        // change the values of inputs         
        wrapper.find('textarea').simulate('change', { target: { value: changedRecipe.instructions } });
    ...
    it('test click event (using mount from @testing-library/react)', async () => {
        ...
        const wrapper = render((<Recipe recipe={recipe} onChange={mockUpdateFunc} onCancel={mockCancelFunc} />));
        // change the values of inputs 
